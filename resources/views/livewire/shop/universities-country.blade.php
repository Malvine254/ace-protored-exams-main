<div>
    <section class="w-full bg-white dark:bg-gray-800 dark:border-t dark:border-t-gray-700 py-6">
        <div class="container md:px-6 lg:px-8">
            ...
            <div class="w-full text-center py-10">
                <h1 class="text-3xl mb-4 font-bold dark:text-white capitalize">Universities in {{ $country }}</h1>
                <p class="max-w-3xl mx-auto text-center text-gray-600 mb-6">
                    Find test banks, solution manuals, and study materials specifically for your university right here
                    on {{ config('app.name') }}! Whether you're looking for resources for your current courses or
                    preparing
                    for
                    exams, our curated selection makes it easy to access the materials you need to succeed. Explore now
    ...
    <section class="w-full container py-6 md:px-6 lg:px-8 md:grid md:grid-cols-[280px_1fr] gap-8">
        <div>
            <h4 class="uppercase text-xs text-gray-600 font-semibold mb-5">Countries</h4>
            @foreach ($countries as $country)
                <a href="/school/{{ kebabCase($country) }}"
        ...
        <div>
            <h4 class="uppercase text-xs text-gray-600 font-semibold mb-5">Universities</h4>

            <div>
                @foreach ($schools as $item)
                    <a href="/school/{{ kebabCase($item->country) }}/{{ kebabCase($item->state_province) }}/{{ kebabCase($item->name) }}"
